if (isServer) then {
	_vehicle_314 = objNull;
	if (true) then
	{
	  _this = createVehicle ["Body1", [5626.4521, 8577.7822, 0], [], 0, "CAN_COLLIDE"];
	  _vehicle_314 = _this;
	  _this setPos [5626.4521, 8577.7822, 0];
	};

	_vehicle_316 = objNull;
	if (true) then
	{
	  _this = createVehicle ["Body1", [5653.271, 8565.1514, -3.0517578e-005], [], 0, "CAN_COLLIDE"];
	  _vehicle_316 = _this;
	  _this setPos [5653.271, 8565.1514, -3.0517578e-005];
	};

	_vehicle_319 = objNull;
	if (true) then
	{
	  _this = createVehicle ["Body2", [5631.9009, 8564.624, 9.1552734e-005], [], 0, "CAN_COLLIDE"];
	  _vehicle_319 = _this;
	  _this setDir 120.97764;
	  _this setPos [5631.9009, 8564.624, 9.1552734e-005];
	};

	_vehicle_321 = objNull;
	if (true) then
	{
	  _this = createVehicle ["Body2", [5626.8066, 8571.2871], [], 0, "CAN_COLLIDE"];
	  _vehicle_321 = _this;
	  _this setDir -6.9082727;
	  _this setPos [5626.8066, 8571.2871];
	};

	_vehicle_323 = objNull;
	if (true) then
	{
	  _this = createVehicle ["Body2", [5624.5215, 8586.6445, -3.0517578e-005], [], 0, "CAN_COLLIDE"];
	  _vehicle_323 = _this;
	  _this setDir -10.941958;
	  _this setPos [5624.5215, 8586.6445, -3.0517578e-005];
	};
};